...
const AddProjectForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg mx-auto p-6 bg-white shadow-md rounded-lg">
      ...
      <div className="mb-4">
        <label htmlFor="projectTitle" className="block text-gray-700 font-semibold mb-2">Project Title*</label>
        <input type="text" id="projectTitle" name="projectTitle" {...register('projectTitle', {required: true})} placeholder="Project Title" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" />
        {errors?.projectTitle && <p className="text-red-500">{errors?.projectTitle.message}</p>}
      </div>

      <div className="mb-4">
        <label htmlFor="projectGitLink" className="block text-gray-700 font-semibold mb-2">Github Link</label>
        <input type="text" id="projectGitLink" name="projectGitLink" {...register('projectGitLink')} placeholder="Enter Github Link" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" />
        {errors?.projectGitLink && <p className="text-red-500">{errors?.projectGitLink.message}</p>}
      </div>

      <div className="mb-4">
        <label htmlFor="projectLiveLink" className="block text-gray-700 font-semibold mb-2">Live Link*</label>
        <input type="text" id="projectLiveLink" name="projectLiveLink" {...register('projectLiveLink', {required: true})} placeholder="Enter Live Link" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" />
        {errors?.projectLiveLink && <p className="text-red-500">{errors?.projectLiveLink.message}</p>}
      </div>

    
      <div className="mb-4">
      <label htmlFor="projectAchievements" className="block text-sm mt-2 p-1 font-medium text-gray-600">
          ...
          render={({ field }) => (
            <div className="my-3 py-2">
              ...
              <TagsInput
                type="text"
                {...field}
                placeholder="Add Project Achievemnts"
